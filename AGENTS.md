...
- The migration history is squashed into a consolidated `supabase/migrations/0001_baseline.sql`, regenerated from the live schema; the baseline folds in everything the former incremental migrations did. New schema changes since land as fresh incremental migrations on top (`0002`+, e.g. `0002` adds `chat_threads.director_notes`). The remote ledger records the baseline plus those incrementals.
- A daily Vercel cron (`vercel.json` → `/api/internal/keepalive`, gated by `CRON_SECRET`) runs a trivial query so the free-tier Supabase project never idles for a week and auto-pauses (a pause takes the whole app down until manually resumed).
- World state is JSONB, not normalized tables. `src/lib/ai/state-materializer.ts` is gone; snapshot reads are a pure blob parse in `src/lib/domain/world-snapshot.ts` (`parseWorldSnapshot`), and mutations apply in memory via `src/lib/domain/world-state-reducer.ts` before one atomic `upsert_world_snapshot` RPC. The legacy normalized `world_*` tables were dropped in favor of the `world_state` JSONB column.
- `profiles` is decoupled from Supabase Auth (no FK to `auth.users`) and the fixed user is seeded directly. Every ownership RPC (`begin_turn`, `commit_turn`, `fail_turn`, `create_branch_from_turn`, `rewind_branch_to_turn`, `activate_branch`, `set_default_persona`, `create_thread_with_branch`) takes an explicit `p_user_id` — `auth.uid()` is NULL under the service-role client, so RPCs must never gate on it.
